...
export type PoetryStatusFilter = Poetry["status"] | "";
export type PoetryFeaturedFilter = "all" | "featured" | "standard";

interface PoetryFiltersProps {
  searchQuery: string;
  setSearchQuery: (value: string) => void;
  selectedCategory: string;
  setSelectedCategory: (value: string) => void;
  selectedStatus?: PoetryStatusFilter;
  setSelectedStatus?: (value: PoetryStatusFilter) => void;
  selectedFeatured?: PoetryFeaturedFilter;
  setSelectedFeatured?: (value: PoetryFeaturedFilter) => void;
  categories: string[];
}

export default function PoetryFilters({
  searchQuery,
  setSearchQuery,
  selectedCategory,
  setSelectedCategory,
  ...
  return (
    <section
      ...
      aria-label="Filter poetry"
    >
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end">
        <div className="min-w-0 flex-1">
          <label
            htmlFor="poetry-search"
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:flex lg:items-end">
          <div className="min-w-40">
            <label
              htmlFor="poetry-category-filter"
              className="mb-2 block text-sm font-medium text-amber-100/80"
            >
              Category
            </label>
            <select
              id="poetry-category-filter"
          ...
          {setSelectedStatus && (
            <div className="min-w-36">
              <label
                htmlFor="poetry-status-filter"
                className="mb-2 block text-sm font-medium text-amber-100/80"
              >
                Status
              </label>
              <select
                id="poetry-status-filter"
                value={selectedStatus}
                onChange={(event) =>
                  setSelectedStatus(event.target.value as PoetryStatusFilter)
                }
                className="w-full cursor-pointer rounded-xl border border-amber-500/20 bg-slate-950/50 px-3 py-3 text-sm text-white transition-colors outline-none hover:border-amber-500/35 focus:border-amber-400 focus:ring-2 focus:ring-amber-400/20"
              >
          ...
          {setSelectedFeatured && (
            <div className="min-w-36">
              <label
                htmlFor="poetry-featured-filter"
                className="mb-2 block text-sm font-medium text-amber-100/80"
              >
                Visibility
              </label>
              <select
                id="poetry-featured-filter"
                value={selectedFeatured}
                onChange={(event) =>
                  setSelectedFeatured(
                    event.target.value as PoetryFeaturedFilter,
                  )
                }
                className="w-full cursor-pointer rounded-xl border border-amber-500/20 bg-slate-950/50 px-3 py-3 text-sm text-white transition-colors outline-none hover:border-amber-500/35 focus:border-amber-400 focus:ring-2 focus:ring-amber-400/20"
              >
